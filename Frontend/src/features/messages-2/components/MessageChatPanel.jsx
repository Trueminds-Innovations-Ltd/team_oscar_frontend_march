...
import { MessageFolderIcon, PaperclipIcon, SendIcon, SmileyIcon } from './MessageIcons';

const bubbleSizeClasses = {
  sm: 'w-[44%] min-w-[140px] max-sm:w-[82%]',
  md: 'w-[58%] min-w-[180px] max-sm:w-[84%]',
  lg: 'w-[72%] min-w-[220px] max-sm:w-[88%]',
  xl: 'h-[116px] w-[68%] min-w-[220px] max-sm:h-auto max-sm:w-[88%]',
};
...
const MessageBubble = ({ message, current }) => {
  ...
  return (
    <div className={`flex items-start gap-3 ${isRight ? 'justify-end' : 'justify-start'}`}>
      {!isRight ? (
        <BubbleAvatar side="left" initials={current.avatar || 'TF'} tone="bg-gradient-to-br from-slate-700 to-slate-500" />
      ) : null}
      ...
      {isRight ? (
        <BubbleAvatar side="right" initials={current.role === 'Tutor' ? 'TJ' : 'SP'} tone="bg-gradient-to-br from-slate-900 to-slate-700" />
      ) : null}
    </div>
  );
};
...
const HeaderAvatar = ({ variant, activeItem }) => {
  ...
  return (
    <div className="flex h-11 w-11 items-center justify-center rounded-full bg-gradient-to-br from-amber-700 to-orange-500 text-sm font-semibold text-white shadow-sm">
...
const MessageChatPanel = ({ variant, activeItem }) => {
  return (
    <section className="flex min-h-[560px] flex-col overflow-hidden rounded-[28px] border border-white/80 bg-white shadow-[0_20px_45px_rgba(15,23,42,0.05)]">
      <div className="flex items-center gap-3 border-b border-slate-200 px-5 py-5 sm:px-6">
        ...
        <div>
          <p className="text-sm font-semibold text-slate-900">
            {variant === 'tickets' ? 'Support' : activeItem.name}
          </p>
          <p className="text-xs text-slate-500">{variant === 'tickets' ? 'Ticket conversation' : activeItem.role || 'Tutor'}</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto bg-[radial-gradient(circle_at_top,rgba(189,245,237,0.22),rgba(255,255,255,0.96)_34%,rgba(226,232,240,0.82)_100%)] px-4 py-6 sm:px-5">
        <div className="space-y-4">
          {activeItem.messages.map((message) => (
            <MessageBubble key={message.id} message={message} current={activeItem} />
          ))}
        </div>
      </div>
      ...
      <div className="border-t border-slate-200 bg-white px-4 py-4 sm:px-5">
        <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-[#f8fafc] px-3 py-3 shadow-inner">
          <button type="button" className="text-slate-400 transition hover:text-slate-600" aria-label="Attach file">
            <PaperclipIcon />
          </button>
          <button type="button" className="text-slate-400 transition hover:text-slate-600" aria-label="Add emoji">
            <SmileyIcon />
          </button>
          <input
            type="text"
